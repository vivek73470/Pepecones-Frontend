...
const ProductGallery = ({
  ...
  const [imageObjects, setImageObjects] = useState([]); // Track uploaded images and their alt tags
  ...
  const handleImageChange = async (event) => {
    ...
    try {
      ...
      // Remove objects with null imageUrl and altTag
      const filteredImageObjects = imageObjects.filter(
        (item) => item.imageUrl !== null && item.altTag !== null,
      );

      // Add the new image object to the filteredImageObjects
      filteredImageObjects.push({ imageUrl, altTag });

      // Update the imageObjects state with the filteredImageObjects
      setImageObjects(filteredImageObjects);

      // Use the imageUrl in your API call or any other logic
  ...
  const addImagesToFormData = (e) => {
    ...
    // Filter the imageObjects array to keep only objects with both imageUrl and altTag
    const filteredImageObjects = imageObjects.filter(
      (item) => item.imageUrl && item.altTag,
    );

    if (filteredImageObjects.length > 0) {
      // Combine the existing imageUrls with the new filtered image objects
      const updatedImageUrls = [...formData.imageUrls, ...filteredImageObjects];

      // Update formData with the modified imageUrls array
      setFormData({
        ...formData,
        imageUrls: updatedImageUrls,
      });

      // Clear the temporary array and reset the input fields
      setImageObjects([]);
      setSelectedImage(null);
      setShowInput(false);
      ...
      toast.error('Please First Write Alt Tag ');
    }
  };
            ...
            {viewGallery ? (
              ...
              >
                <div className="gallery-section">
                  {formData.imageUrls.map((imageObject, index) => (
